...
class TechnicalAnalysisStrategy(TradingStrategy):
    ...
    def detect_triangle_patterns(self, df, period=20):
        ...
        if abs(high_slope) > 0 and abs(low_slope) > 0:
            if high_slope < 0 and low_slope > 0 and abs(high_slope/low_slope) < 3.0:  # Loosened for more detections
                patterns['symmetrical_triangle'] = True
        
        # Ascending Triangle: horizontal resistance, rising support
        high_std = np.std(highs)
        if high_std < np.std(highs) * 0.8 and low_slope > 0:  # Loosened from 0.7
            patterns['ascending_triangle'] = True
        
        # Descending Triangle: horizontal support, falling resistance
        low_std = np.std(lows)
        if low_std < np.std(lows) * 0.8 and high_slope < 0:  # Loosened from 0.7
    ...
    def detect_channel_wedge_patterns(self, df, period=20):
        ...
        if abs(high_slope) < 0.002 and abs(low_slope) < 0.002:  # Loosened for more detections
            patterns['ranging_channel'] = True
            
        # Rising Wedge: highs rising faster than lows
        if high_slope > 0 and low_slope > 0 and high_slope > low_slope * 1.2:  # Loosened from 1.5
            patterns['rising_wedge'] = True
            
        # Falling Wedge: lows falling faster than highs
        if high_slope < 0 and low_slope < 0 and abs(low_slope) > abs(high_slope) * 1.2:  # Loosened from 1.5
    ...
    def detect_harmonic_patterns(self, df, period=50):
        ...
        if abs(price_change) < 0.07:  # Loosened for more detections
            patterns['gartley'] = True
        elif 0.07 <= abs(price_change) < 0.12:
            patterns['bat'] = True
        elif 0.12 <= abs(price_change) < 0.17:
            patterns['butterfly'] = True
        elif 0.17 <= abs(price_change) < 0.22:
            patterns['crab'] = True
        else:
            patterns['shark'] = True
    ...
    def analyze(self, df):
        ...
        if channel_wedge_patterns['ranging_channel']:
            pattern_score += 1  # This gives the pattern_score 1 as in your example
